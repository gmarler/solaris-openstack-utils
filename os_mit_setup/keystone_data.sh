...
openstack service create --name=nova \
                         --description="Nova Compute Service V2" \
                         computev2
if [[ -z "$DISABLE_ENDPOINTS" ]]; then
    openstack endpoint create --region $REGION_NAME \
	--publicurl "$PROTOCOL://$NOVA_PUBLIC_ADDRESS:8774/v2/\$(tenant_id)s" \
...
openstack service create --name=cinder \
    --description="Cinder Volume Service (Version 1)" \
                         volume
if [[ -z "$DISABLE_ENDPOINTS" ]]; then
    openstack endpoint create --region $REGION_NAME \
...
openstack service create --name=cinderv3 \
                         --description="Cinder Volume Service (Version 3)" \
                         volumev3
if [[ -z "$DISABLE_ENDPOINTS" ]]; then
    openstack endpoint create --region $REGION_NAME \
	--publicurl "$PROTOCOL://$CINDER_PUBLIC_ADDRESS:8776/v3/\$(tenant_id)s" \
	--adminurl "$PROTOCOL://$CINDER_ADMIN_ADDRESS:8776/v3/\$(tenant_id)s" \
	--internalurl "$PROTOCOL://$CINDER_INTERNAL_ADDRESS:8776/v3/\$(tenant_id)s" \
        volumev3
fi
...
# Ironic service
#
# Ironic Eventlet server does not support SSL/TLS endpoint
...
if [[ -z "$DISABLE_ENDPOINTS" ]]; then
    openstack endpoint create --region $REGION_NAME \
	--publicurl "$PROTOCOL://$IRONIC_PUBLIC_ADDRESS:6385" \
	--adminurl "$PROTOCOL://$IRONIC_ADMIN_ADDRESS:6385" \
	--internalurl "$PROTOCOL://$IRONIC_INTERNAL_ADDRESS:6385" \
        ironic
fi
PROTOCOL=$PROTOCOLB
