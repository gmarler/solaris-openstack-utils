...
if [ -z "$keyr" ]; then
	...
	if [ $? -ne  0 ]; then
        ...
        mkdir -p ~/.ssh
        cat $keypairpath >> ~/.ssh/known_hosts
fi
...
for img in $imagel
do
        ishowout=$(openstack image show $img)
        iarch=$(echo "$ishowout" | grep properties | sed '/^+/ d' | sed 's/|//g' | sed "s/.*architecture='//g" | sed "s/',.*//g")
	iname=$(echo "$ishowout" | sed '/^+/ d' | sed 's/|//g' | grep name | awk '{print $2}')
...
openstack stack create --wait -t ./heat-test.yaml --parameter "flavor=$flavor;key_name=$keypairname;image=$testimg;public_net=external;private_net=proj_0001_internal" $stack_name
